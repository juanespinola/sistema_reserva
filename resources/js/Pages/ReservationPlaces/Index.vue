<template>
    <jig-layout>
        <template #header>
            <div class="flex flex-wrap items-center justify-between w-full px-4">
                ...
                <div class="flex gap-x-2">
                    <inertia-button v-if="can.create" :href="route('admin.reservation-places.create')" classes="bg-green-100 hover:bg-green-200 text-primary"><i class="fas fa-plus"></i> New
                        Reservation Place</inertia-button>
            ...
            <div class="z-10 flex-auto bg-white md:rounded-md md:shadow-md">
                ...
                <div v-if="showModal && currentModel">
                    <jig-modal
                        ...
                        <template #title>Show Reservation Place #{{currentModel.id}}</template>
                        <show-reservation-places-form :model="currentModel"></show-reservation-places-form>
                        <template #footer>
        ...
        <div v-else class="p-4 font-bold text-red-500 bg-red-100 rounded-md shadow-md ">
            You are not authorized to view a list of Reservation Places
        </div>
    </jig-layout>
</template>
...
<script>
    ...
    import ShowReservationPlacesForm from "@/Pages/ReservationPlaces/ShowForm.vue";
    ...
    export default defineComponent({
        ...
        computed: {
            ajaxUrl() {
                const url = new URL(this.route('api.reservation-places.dt'));
        ...
        methods: {
            showModel(model) {
                axios.get(route('api.reservation-places.show',model)).then(res => {
                    this.currentModel = res.data.payload;
                    this.showModal = true;
                })
                // this.$inertia.visit(this.route('admin.reservation-places.show',model.id));
            },
            editModel(model) {
                this.$inertia.visit(this.route('admin.reservation-places.edit',model.id));
            },
            ...
            async deleteModel() {
                ...
                if (this.currentModel) {
                    this.$inertia.delete(route('admin.reservation-places.destroy', vm.currentModel),{
                        onFinish: res => {
            ...
            async toggleActivation(enabled,model) {
                const vm = this;
                console.log(enabled);
                axios.put(route(`api.reservation-places.update`,model.id),{
                    enabled: enabled
                }).then(res => {
