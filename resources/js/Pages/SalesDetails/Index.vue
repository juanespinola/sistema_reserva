<template>
    <jig-layout>
        <template #header>
            <div class="flex flex-wrap items-center justify-between w-full px-4">
                ...
                <div class="flex gap-x-2">
                    <inertia-button v-if="can.create" :href="route('admin.sales-details.create')" classes="bg-green-100 hover:bg-green-200 text-primary"><i class="fas fa-plus"></i> New
                        Sales Detail</inertia-button>
            ...
            <div class="z-10 flex-auto bg-white md:rounded-md md:shadow-md">
                ...
                <div v-if="showModal && currentModel">
                    <jig-modal
                        ...
                        <template #title>Show Sales Detail #{{currentModel.id}}</template>
                        <show-sales-details-form :model="currentModel"></show-sales-details-form>
                        <template #footer>
        ...
        <div v-else class="p-4 font-bold text-red-500 bg-red-100 rounded-md shadow-md ">
            You are not authorized to view a list of Sales Detail
        </div>
    </jig-layout>
</template>
...
<script>
    ...
    import ShowSalesDetailsForm from "@/Pages/SalesDetails/ShowForm.vue";
    ...
    export default defineComponent({
        ...
        computed: {
            ajaxUrl() {
                const url = new URL(this.route('api.sales-details.dt'));
        ...
        methods: {
            showModel(model) {
                axios.get(route('api.sales-details.show',model)).then(res => {
                    this.currentModel = res.data.payload;
                    this.showModal = true;
                })
                // this.$inertia.visit(this.route('admin.sales-details.show',model.id));
            },
            editModel(model) {
                this.$inertia.visit(this.route('admin.sales-details.edit',model.id));
            },
            ...
            async deleteModel() {
                ...
                if (this.currentModel) {
                    this.$inertia.delete(route('admin.sales-details.destroy', vm.currentModel),{
                        onFinish: res => {
            ...
            async toggleActivation(enabled,model) {
                const vm = this;
                console.log(enabled);
                axios.put(route(`api.sales-details.update`,model.id),{
                    enabled: enabled
                }).then(res => {
